...
const TabContainer = (props) => {
  ...
  useEffect(() => {
    ...
    setTimeout(() => {
      ...
      if (pane && pane.key !== activeKey) setActiveKey(pane.key);
    }, 100);
  }, [pathname]);
  ...
  useEffect(() => {
    ...
    // 同步menu位置和page title
    if (pane) history.replace(pane.pathname);
  ...
  const addChildren = () => {
    if (children) {
      const pane = panes.find(
        (item) => item.pathname === children.props.location.pathname
      );
      if (pane) {
        setComponents({
          ...components,
          [pane.key]: (
            <RefreshComponent renderKey={pane.randomKey}>
              {children}
            </RefreshComponent>
          ),
        });
  ...
  const onRefresh = (pane) => {
    setPanes(
      panes.map((item) =>
        item.key === pane.key ? { ...pane, randomKey: Math.random() } : item
      )
    );
  };
    ...
    <RouteContext.Consumer>
      {(value: any) => {
        ...
        if (location.pathname.match(/login/))
          store.set("user-settings", { reset: true });

        return isIgnorePath(location.pathname) ? (
          children
        ) : (
          <TabContainer title={value.title} location={location}>
            {children}
          </TabContainer>
        );
        // return children;
